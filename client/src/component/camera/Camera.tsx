    ...
    try {
      Tesseract.recognize(picture, "kor").then(async ({ data: { text } }) => {
        ...
        const medi_code = body.items?.item?.itemSeq._text || "",
          ...
          medi_times =
            timeNumStr.length > 1 && timeNumStr[1].length > 0
              ? timeNumStr[1][0]
              : 0,
          medi_num =
            timeNumStr.length > 1 && timeNumStr[1].length > 3
              ? timeNumStr[1][3]
              : 0;

        if (header.resultCode._text == "00") {
          if (medi_code === "" || medi_name === "")
            return alert("지원하지 않는 약봉투 양식입니다.");
          navigate("/user/create-medi-info", {
            state: {
              medi_code,
              medi_name,
              medi_times,
